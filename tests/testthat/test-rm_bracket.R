...
test_that("rm_bracket is removing bracketed text",{
    ...
    expect_true(identical(rm_bracket(examp$text, pattern = "square"), expected1[[1]]))
    expect_true(identical(rm_bracket(examp$text, pattern = "curly"), expected1[[2]]))
    expect_true(identical(rm_bracket(examp$text, pattern = c("square", "round")), expected1 [[3]]))
    expect_true(identical(rm_bracket(examp$text), expected1 [[4]]))
})
...
test_that("rm_bracket is extracting bracketed text",{
    ...
    expect_true(identical(rm_bracket(examp$text, extract=TRUE, pattern = "square"), expected2[[1]]))
    expect_true(identical(rm_bracket(examp$text, extract=TRUE, pattern = "curly"), expected2[[2]]))
    expect_true(identical(rm_bracket(examp$text, extract=TRUE, pattern = c("square", "round")), expected2 [[3]]))
    expect_true(identical(rm_bracket(examp$text, extract=TRUE, pattern = c("square", "round"), merge = FALSE), expected2 [[4]]))
    expect_true(identical(rm_bracket(examp$text, extract=TRUE), expected2[[5]]))
    expect_true(identical(rm_bracket(examp$text, include.bracket = TRUE, extract=TRUE), expected2[[6]]))
