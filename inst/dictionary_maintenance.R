...
regex_usa <- list(
    ...
    rm_endmark3 = "[\\!\\*\\.\\;\\?\\|\\:]+$",    
    rm_hash = "(?<!/)((#)(\\w+))", 
    rm_nchar_words = "(?<![\\p{L}'])(?:'?\\p{L}'?){%s}(?![\\p{L}'])",#"(?<![\\w'])(?:'?\\w'?){%s}(?![\\w'])",
    rm_nchar_words2 = "(?<![\\p{L}'])([\\p{L}']){%s}(?![\\p{L}'])", #"(?<![\\w'])([\\w']){%s}(?![\\w'])",
...
regex_supplement <- list(
    ...
    word_boundary = "(?<=^|[^A-Za-z'])(%s)(?=$|[^A-Za-z'])",
    word_boundary_left = "(?<=^|[^A-Za-z'])",
    word_boundary_right = "(?=$|[^A-Za-z'])",	
